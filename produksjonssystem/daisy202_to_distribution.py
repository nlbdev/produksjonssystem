...
class Daisy202ToDistribution(Pipeline):
    ...
    def on_book(self):
        ...
        accepted_second_head = ["opphavsrett", "copyright", "lydbokavtalen", "audiobook agreement", "the audiobook agreement", "tigar announcement", "nlb", "tibi", "jietnagirjesoahpamuš"]

        if first_head_class != "title":
            self.utils.report.error(f"{edition_identifier} første heading {first_head_class} er ikke title")
            return False

        requires_lydbokavtalen = True
        if edition_metadata["watermark"] is False:
            ...
            # newspapers are not required to have Lydbokavtalen
            requires_lydbokavtalen = False

        if requires_lydbokavtalen and second_head not in accepted_second_head:
            ...
            if os.path.isdir(file_book_path):
                ...
                if file_book.startswith("temp"):
                    ...
                    if file_size >= 681574400:
                        self.utils.report.error(f"Boka {edition_identifier} har en lydfil ({file_book}) som er større enn 650MB")
                        return False
