...
class GenerateResources(Pipeline):
    ...
    def _trigger_resource_thread(self):
        ...
        while self._dirsAvailable and self._shouldRun:
            ...
            if 'url_database_list' not in self.config:
                logging.info("Finner ikke url til DOD api for liste over bøker")
                continue

            if "url_regenerate_resources" not in self.config:
                logging.info("Finner ikke url til ressursoppdatering")
                continue
