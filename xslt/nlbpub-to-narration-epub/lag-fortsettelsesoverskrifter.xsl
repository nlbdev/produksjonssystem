...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:message>lag-fortsettelsesoverskrifter.xsl (0.9.1 / 2018-04-04)</xsl:message>
        ...
        <xsl:message>
            ...
            <xsl:value-of select="count(//section/section[following-sibling::element() and not(local-name(following-sibling::element()[1]) eq 'section')])"/>
    ...
    <xsl:template match="section/section">
        ...
        <xsl:if
            test="following-sibling::element() and not(local-name(following-sibling::element()[1]) eq 'section')">
            ...
            <xsl:variable name="FO" as="xs:string">
                <xsl:choose>
                    <xsl:when test="starts-with(//meta[@name eq 'dc:language'][1]/@content,'en')">
                        <xsl:value-of select="concat('Resuming the heading ''', ancestor::section[1]/child::element()[matches(local-name(), '^h[1-6]$')], ''':')"/>
                    </xsl:when>
                    <xsl:when test="matches(//meta[@name eq 'dc:language'][1]/@content,'^(nn-no|nn)$','i')">
                        <xsl:value-of select="concat('Framhald av overskrifta «', ancestor::section[1]/child::element()[matches(local-name(), '^h[1-6]$')], '»:')"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="concat('Fortsettelse på overskriften «', ancestor::section[1]/child::element()[matches(local-name(), '^h[1-6]$')], '»:')"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
...
<!--            <xsl:message>FO: <xsl:value-of select="$FO"/></xsl:message>
            ...
            <section>
                ...
                <xsl:element name="{concat('h',string($nivå))}">
                    ...
                    <xsl:attribute name="id" select="concat('nlb-fo-h-section-',generate-id())"></xsl:attribute>
