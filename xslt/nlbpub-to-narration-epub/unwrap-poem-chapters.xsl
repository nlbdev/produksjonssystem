...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="section[count(section) = 1
                            and (section[f:types(.) = ('z3998:poem', 'poem')] or section[f:types(.) = ('z3998:verse') and not(parent::section[f:types(.) = ('z3998:poem', 'poem')])])
        ...
        <xsl:copy exclude-result-prefixes="#all">
            ...
            <xsl:attribute name="epub:type" select="string-join((f:types(.)[. = ('cover', 'frontmatter', 'bodymatter', 'backmatter')], f:types(section)[not(. = ('z3998:verse', 'z3998:poem'))], 'z3998:poem'), ' ')"/>
