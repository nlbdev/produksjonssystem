...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="create-copyright-page">
        ...
        <xsl:variable name="narrator" select="ancestor::html/head/meta[@name='dc:contributor.narrator']/string(@content)" as="xs:string?"/>
        <xsl:variable name="narrator" select="if (boolean($narrator) and $narrator != 'Talesyntese') then tokenize($narrator, '\s+')[1] else
              ...
              <xsl:choose>
                  <xsl:when test="$language = ('en', 'eng')">
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                          ...
                          <p>
                              This digital talking book is narrated by the synthetic voice <xsl:value-of select="$narrator"/> and produced as a DAISY 2.02 fulltext audio book.
                  ...
                  <xsl:when test="$language = ('nb', 'nob', 'no')">
                    <section epub:type="frontmatter copyright-page" id="copyright-section-{generate-id()}">
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                        ...
                        <p>
                            DAISY-boka er lest av den syntetiske stemmen <xsl:value-of select="$narrator"/>, og produsert som en DAISY 2.02 fulltekst lydbok. 
                  ...
                  <xsl:when test="$language = ('se', 'sme')">
                      <section epub:type="frontmatter copyright-page" id="copyright-section-{generate-id()}">
                          ...
                          <p>
                              Dán veršuvdna lea NLB buvttadan jagi <xsl:value-of select="$year"/> vuoigŋaduodjelága § 55 mielde, ja dan oažžu dušše fal priváhta atnui máŋget.
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                          ...
                          <p>
                              DAISY-girjji lea lohkan syntehtalaš jietna <xsl:value-of select="$narrator"/>, ja girji lea buvttaduvvon DAISY 2.02 ollesteavstta jietnagirjin.
                              ...
                              Sáddestehket fal e-poastta deike: talesyntese@nlb.no. Čilges mii sániid lea jietnaduvvon boastut, ja olles cealkaga mas sátni lea oassin. Ovddalgihtii giitu!
                          </p>
                          <figure class="image"><img alt="NLB logo" src="images/{upper-case($library)}_logo.png"/></figure>
                      </section>
                  </xsl:when>
                  ...
                  <xsl:otherwise>
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                          <h1 id="bookinfo-headline-{generate-id()}">Bokinformasjon</h1>
                          <p>
                              DAISY-boka er lesen av den syntetiske stemma <xsl:value-of select="$narrator"/>, og produsert som ei DAISY 2.02 fulltekst lydbok. 
