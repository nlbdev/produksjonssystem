              ...
              <xsl:choose>
                  <xsl:when test="$language = ('en', 'eng')">
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                          ...
                          <p>
                              This digital talking book is narrated by the synthetic <xsl:value-of select="if (count($narrators) gt 1) then 'voices' else 'voice'"/> <xsl:value-of select="string-join($narrators, ' and ')"/> and produced as a DAISY 2.02 fulltext audio book.
                  ...
                  <xsl:when test="$language = ('nb', 'nob', 'no')">
                    <section epub:type="frontmatter copyright-page" id="copyright-section-{generate-id()}">
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                        ...
                        <p>
                            DAISY-boka er lest av <xsl:value-of select="if (count($narrators) gt 1) then 'de syntetiske stemmene' else 'den syntetiske stemmen'"/> <xsl:value-of select="string-join($narrators, ' og ')"/>, og produsert som en DAISY 2.02 fulltekst lydbok. 
                  ...
                  <xsl:otherwise>
                      ...
                      <section epub:type="frontmatter copyright-page" id="bookinfo-section-{generate-id()}">
                          <h1 id="bookinfo-headline-{generate-id()}">Bokinformasjon</h1>
                          <p>
                              DAISY-boka er lesen av <xsl:value-of select="if (count($narrators) gt 1) then 'dei syntetiske stemmane' else 'den syntetiske stemma'"/> <xsl:value-of select="string-join($narrators, ' og ')"/>, og produsert som ei DAISY 2.02 fulltekst lydbok. 
